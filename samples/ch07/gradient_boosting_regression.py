...
def train_and_predict (X_train, train_y, X_test, max_depth, n_estimators):
    # train model
...
def fit_it (max_depth, n_estimators):
    train_y, test_x, X_train, X_test = get_input ()
    test_predicted = train_and_predict (X_train, train_y, X_test, max_depth, n_estimators)
...
def worker (max_depth, n_estimators):
...
def plot_bias_variance (M, max_depth, bias_2, variance): 
    ...
    '''
def driver (N_data_points, L_datasets, noise_coefficient, n_estimators):

    init (N_data_points, L_datasets, noise_coefficient)
    
    for m in range (2, M + 1, 1):
        max_depth.append (m)
        b2, var = worker (m, n_estimators)
        bias_2.append (b2)
        variance.append (var)
        total.append (b2 + var)
...
driver (N_data_points, L_datasets, noise_coefficient, n_estimators)
print("n_estimators =  " + str(n_estimators) + " total_time = " + str(time.time() - start) + " seconds")
plt.show()