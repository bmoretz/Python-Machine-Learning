...
sgd = SGDClassifier(loss='log', tol = 1e-3)
sgd_a = SGDClassifier(loss='log', average = True, tol = 1e-3)
lr = LogisticRegression(tol = 1e-3)
dt = DecisionTreeClassifier(max_depth=10, criterion='entropy')
linear_SVC = CalibratedClassifierCV(LinearSVC(tol = 1e-3))
VC_linear_kernel = SVC(probability=True, kernel='linear', tol = 1e-3)
SVC_rbf_kernel = SVC(probability=True, kernel='rbf', tol = 1e-3)
...
voting_clf = VotingClassifier(estimators=individual_clfs, voting='soft')
